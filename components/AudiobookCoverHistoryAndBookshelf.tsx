...
import { updateIfBookShelvedDB, isAudiobookFullyDownloaded, storeAsyncData } from "../db/database_functions";
...
export default function AudiobookCover(props) {
  ...
  const isCurrentBookLoaded = audio.currentBook?.audioBookId === item?.audiobook_id;
  ...
  const openReviews = async () => {
    const url = item?.audiobook_review_url;
    if (!url) return;
  ...
  const openInfo = async () => {
    ...
    try {
      const res = await fetch(`https://librivox.org/api/feed/audiobooks/?id=${item?.audiobook_id}&fields={description}&format=json`);
      const json = await res.json();
  ...
  return (
    ...
    >
      ...
      >
        {/* Playing indicator overlay — doesn't affect layout */}
        {isCurrentBookLoaded && (
          <View
            pointerEvents="none"
            style={{
        ...
        >
          ...
          <TouchableOpacity
            ...
            accessibilityLabel={isCurrentlyPlaying ? `Pause ${item.audiobook_title}` : `Play ${item.audiobook_title}`}
            style={{
              position: "absolute",
              bottom: 10,
              right: 2,
